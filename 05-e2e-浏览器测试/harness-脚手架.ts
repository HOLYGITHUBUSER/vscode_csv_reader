 ...
 * the real `02-webview-表格界面/webview-main.js`. Loaded via `file://` — NO Node, NO jsdom,
...
<!-- Context menu + compact filter footer + Find widget. This mirrors the current
     provider markup: column filters are opened from each header. -->
...
<div id="csvFloatPanel" style="position:fixed;right:16px;bottom:16px;z-index:1150;display:flex;align-items:center;flex-wrap:nowrap;gap:8px;max-width:calc(100vw - 32px);overflow-x:auto;overflow-y:hidden;white-space:nowrap;padding:6px 10px;border:1px solid #ccc;border-radius:6px;background:#fff;">
  ...
  <span id="csvPanelDivider" hidden style="width:1px;height:18px;background:#ccc;margin:0 2px;"></span>
 ...
 *   - inlines 02-webview-表格界面/webview-main.js so Chromium runs the same bytes the webview
 *     would run
 * Returns the file:// URL to load.
 */
export function writeHarnessHtml(cfg: HarnessConfig): { url: string; dir: string } {
  const mainJs = fs.readFileSync(path.join(REPO_ROOT, '02-webview-表格界面', 'webview-main.js'), 'utf8');
  const findReplaceJs = fs.readFileSync(path.join(REPO_ROOT, '02-webview-表格界面', 'webview-find-replace.js'), 'utf8');
  const filterPanelJs = fs.readFileSync(path.join(REPO_ROOT, '02-webview-表格界面', 'webview-filter-panel.js'), 'utf8');
