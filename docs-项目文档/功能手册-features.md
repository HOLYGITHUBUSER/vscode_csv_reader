...
> **自动化测试**：见 `extension-扩展逻辑/test/webview-column-filter.test.ts`、`extension-扩展逻辑/test/provider-utils.test.ts` 和 `e2e-浏览器测试/float-panel.spec.ts`。
...
状态机单测在 `extension-扩展逻辑/test/sort-snapshot.test.ts`，jsdom 级交互在 `extension-扩展逻辑/test/webview-sort-button.test.ts`，Playwright 真浏览器在 `e2e-浏览器测试/sort-tri-state.spec.ts`。
...
所有 per-file 设置都挂在 `CsvEditorProvider` 的 per-URI state 里；细节见 `extension-扩展逻辑/test/provider-state.test.ts`。
