...
extension-扩展逻辑/**                      源码（只发编译好的 out/）
langConfig-语言配置/eslint-*.mjs         lint 配置
langConfig-语言配置/vscode-test-*.mjs    测试配置（不是运行时必需）
scripts/**                       构建脚本
backup-归档旧文件/**                        归档
docs-项目文档/**                     中文文档（装扩展的人不需要）
artifacts-安装包/**                     之前的包（不让套娃）
.gitignore / .yarnrc / tsconfig / eslint.config / *.ts / *.map
out/test/**                     测试产物
icon-扩展图标/Screenshot_*.png    截图
icon-扩展图标/*.orig.png          图标原图（保留不处理的那份不进 VSIX）
samples-试用样例/**                    人眼试用样例
e2e-浏览器测试/**                   E2E 测试
```
...
- `out/`（编译后的扩展 JS）
- `webview-表格界面/main.js`
- `icon-扩展图标/icon.png`
- `langConfig-语言配置/language-语言配置.json`
- `package.json` / `README.md` / `LICENSE`

打包完建议去 `artifacts-安装包/BUILD-INFO.md` 确认"文件大小"在 1.4 MB 上下——显著变大基本是没排对某个资产。

### 检查 VSIX 真的排对了

```bash
unzip -l artifacts-安装包/csv-custom-pro-latest.vsix | head -30
```
...
# 普通 VS Code
code --install-extension artifacts-安装包/csv-custom-pro-latest.vsix --force
```
