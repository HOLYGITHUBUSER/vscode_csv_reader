...
- 新增 `e2e-浏览器测试/float-panel.spec.ts` —— Playwright 真浏览器 4 条断言，全部在真点击 + 键盘事件下覆盖上面 4 条行为。
...
- 新增 `e2e-浏览器测试/` Playwright 真浏览器测试脚手架，不再只靠 jsdom：
  - `harness.ts` 生成自包含 HTML（内联 `webview-表格界面/main.js` + shim `acquireVsCodeApi`），通过 `file://` 让 Chromium 直接跑真前端。
...
- 新增 `extension-扩展逻辑/test/sort-snapshot.test.ts`（状态机单测）。
...
- 引入 `extension-扩展逻辑/test/helpers/webview-harness.ts` —— jsdom 版 webview 测试脚手架。
...
- 在 README 顶部加入扩展图标。
- 新增 `samples-试用样例/`（`complex_test.csv` + `super_example.csv`）专门给"肉眼打开试用"，与 `extension-扩展逻辑/test/` 下给自动化的严格样本分开。
- 修复了 `scripts/gen-huge-fixtures.py` 会把 `samples-试用样例/` 覆盖成几十 MB 非法 CSV 的隐患（触发 VS Code 内部 `Assertion Failed`）。
...
- 目录改为 "英文-中文" 双轨命名：`extension-扩展逻辑/`、`webview-表格界面/`、`artifacts-安装包/`、`icon-扩展图标/`、`langConfig-语言配置/`、`samples-试用样例/`、`docs-项目文档/`。
...
- 更新 `.vscodeignore`，显著缩减 VSIX 体积；把 `icon.orig.png`、`samples-试用样例/**`、`e2e-浏览器测试/**`、`scripts/**`、`backup-归档旧文件/**` 等排除。
