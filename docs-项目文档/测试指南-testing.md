...
```
   ...
   慢 ↑  │ Playwright 真浏览器 E2E     │  真 Chromium 点按钮 / 敲键盘
         │ e2e-浏览器测试/*.spec.ts         │  断言 DOM 与 postMessage
         ├─────────────────────────────┤
         │ jsdom Webview 交互单测      │  模拟 main.js 的 DOM 行为
         │ extension-扩展逻辑/test/webview-*.ts   │  几十毫秒一条
         ├─────────────────────────────┤
   快 ↓  │ Node 纯函数单测             │  sort/编辑/权限等
         │ extension-扩展逻辑/test/其它 *.test.ts │
         └─────────────────────────────┘
```
...
npm run install:cursor   # 把 artifacts-安装包/csv-custom-pro-latest.vsix 安装到 Cursor
```

约定：**每次改 `webview-表格界面/main.js` 或 `extension-扩展逻辑/CsvEditorProvider.ts` 的消息分发前后，两个命令都要绿。**
...
3. 打开 `samples-试用样例/ultimate-50mb-完整压力测试.csv`。默认 `csv.maxFileSizeMB` 是 100，因此 50 MiB 样例不应弹大文件拦截。
4. 验证首屏能打开并滚动加载；在右下角列过滤构建器里连续添加两个列条件；确认不会整窗卡死。
5. 验证排序、行高 `紧凑 ↔ 自然折行`、单元格编辑保存都可用。
6. 如果要在 Windsurf 验证，使用 Windsurf 自带 CLI 或 UI 的 "Install from VSIX..." 安装同一个 `artifacts-安装包/csv-custom-pro-latest.vsix`。

## 第 1 层：Node 纯函数单测

位置：`extension-扩展逻辑/test/` 下除 `webview-*` 以外的 `*.test.ts`，例如：
...
位置：`extension-扩展逻辑/test/webview-*.test.ts`，共用脚手架 `extension-扩展逻辑/test/helpers/webview-harness.ts`。
...
1. 读 `webview-表格界面/main.js` 的完整源码。
...
2. 看 `e2e-浏览器测试/test-results/<case>/test-failed-1.png` 失败截图，判断 DOM 长成什么样。
3. `npx playwright show-trace e2e-浏览器测试/test-results/<case>/trace.zip` —— 会启一个本地 UI，能按步骤回放、看每个动作前后的 DOM snapshot，甚至网络请求。
4. 想"肉眼盯着浏览器看怎么跑"：

```bash
npx playwright test \
    --config e2e-浏览器测试/playwright.config.ts \
    --headed --slowmo 500
```
