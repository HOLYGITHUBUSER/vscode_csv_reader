...
| 脚本                    | 干什么                                                                 | 何时用                 |
| --------------------- | ------------------------------------------------------------------- | ------------------- |
| `npm run compile`     | `tsc -p ./`，把 `extension-扩展逻辑/` 编译到 `out/`                                  | 修完 TS 想手动检查         |
| `npm run compile:bump`| 把 `package.json` 的 patch 号 +1 再编译                                   | 改完要试装之前             |
| `npm run lint`        | ESLint 扫 `extension-扩展逻辑/**/*.ts`                                           | CI / 提交前            |
...
| `npm run test:e2e`    | Playwright 真 Chromium 跑 `e2e-浏览器测试/*.spec.ts`                         | 改完 `webview-表格界面/main.js` 必跑 |
| `npm run package`     | 打一个时间戳 VSIX 到 `artifacts-安装包/`，顺带刷 `latest.vsix` + `BUILD-INFO.md`        | 只想打包不升版本            |
...
- **扩展侧逻辑（解析 / 读写文档 / 接收 webview 消息）** → `extension-扩展逻辑/CsvEditorProvider.ts`
- **命令面板的命令** → `extension-扩展逻辑/commands.ts`
- **webview 前端渲染 / 交互** → `webview-表格界面/main.js`
- **新 `csv.*` 配置项** → `package.json` 的 `contributes.configuration` + provider 里消费它
- **给用户试的新样例 CSV** → `samples-试用样例/` + 更新 `samples-试用样例/README.md`
- **给 CI/回归用的严格样本** → `extension-扩展逻辑/test/` + 相应 `*.test.ts`

webview 和扩展之间**只有 `postMessage` 一条沟通线路**。找不到"面板按钮不响应"这类问题时，先看两头：
- 前端 → `webview-表格界面/main.js` 里有没有 `addEventListener` 发消息；
- 后端 → `extension-扩展逻辑/CsvEditorProvider.ts::onDidReceiveMessage` 里的 `case` 分支。
...
4. 在那个窗口里随便打开 `samples-试用样例/ultimate-50mb-完整压力测试.csv` 试交互。
...
1. 把设计稿 / 原图放成 `icon-扩展图标/icon.orig.png`。
2. 运行 `python3 scripts/round-icon.py`，会生成 1024×1024 圆角的 `icon-扩展图标/icon.png`。
...
- **"e2e 一直超时"**：去 `e2e-浏览器测试/test-results/<case>/test-failed-1.png` 看截图，大概率是新 DOM 没加到 `harness.ts` 的骨架里。
