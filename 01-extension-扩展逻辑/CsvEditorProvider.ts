...
class CsvEditorController {
  ...
  public async resolveCustomEditor(
    ...
    webviewPanel.webview.options = {
      ...
      localResourceRoots: [vscode.Uri.file(path.join(this.context.extensionPath, '02-webview-表格界面'))]
    };
  ...
  private wrapHtml(args: {
    ...
    const scriptUri = webview.asWebviewUri(
      vscode.Uri.file(path.join(this.context.extensionPath, '02-webview-表格界面', 'webview-main.js'))
    );
    const findReplaceScriptUri = webview.asWebviewUri(
      vscode.Uri.file(path.join(this.context.extensionPath, '02-webview-表格界面', 'webview-find-replace.js'))
    );
    const filterPanelScriptUri = webview.asWebviewUri(
      vscode.Uri.file(path.join(this.context.extensionPath, '02-webview-表格界面', 'webview-filter-panel.js'))
    );
