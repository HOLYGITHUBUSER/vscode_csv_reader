...
describe('Webview custom find integration', () => {
  const extensionSource = fs.readFileSync(path.join(process.cwd(), '01-extension-扩展逻辑', 'extension.ts'), 'utf8');
  const providerSource = fs.readFileSync(path.join(process.cwd(), '01-extension-扩展逻辑', 'CsvEditorProvider.ts'), 'utf8');
  const webviewScript = fs.readFileSync(path.join(process.cwd(), '02-webview-表格界面', 'webview-main.js'), 'utf8');
  const findReplaceScript = fs.readFileSync(path.join(process.cwd(), '02-webview-表格界面', 'webview-find-replace.js'), 'utf8');
