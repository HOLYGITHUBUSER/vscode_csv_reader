...
describe('Webview find/replace widget', () => {
  ...
  const webviewSource = fs.readFileSync(path.join(process.cwd(), '02-webview-表格界面', 'webview-main.js'), 'utf8');
  const findReplaceSource = fs.readFileSync(path.join(process.cwd(), '02-webview-表格界面', 'webview-find-replace.js'), 'utf8');
