...
| 层级 | 范式 | 示例 |
| --- | --- | --- |
| 顶层目录 | `{两位序号}-{en-kebab}-{中文职责}` | `01-extension-扩展逻辑/` |
| 人读文档 | `{en}-{中文}.md` | `product-产品设计.md` |
| 构建脚本（目录 `03-script-构建脚本/`） | `{en-kebab}-{中文}.{ext}` | `package-vsix-打包.cjs` |
...
- **序号**：两位数字 `01`…`07` 控制资源管理器排序；归档用 `99`。
- 英文段：小写 kebab-case。
- 中文段：2～6 字说明职责。
- 禁止无编号顶层业务目录；禁止 camelCase 目录名。
...
06-docs-项目文档/          # 文档
07-artifacts-安装包/       # 产物
99-backup-归档旧文件/      # 死代码（本地/隐藏）
icon.png
language-语言配置.json
```

`node_modules` / `out` / `99-backup-*` 等用 `files.exclude` 隐藏。
...
- 测试：`01-extension-扩展逻辑/test/` 下，文件名可用双语后缀标明主题
...
- **工程/构建脚本**只放 `03-script-构建脚本/`（版本递增、打包等），双语名。
- 产物只进 `07-artifacts-安装包/`：
  - 稳定名：`csv-custom-pro-latest.vsix`
  - 时间戳名本地临时，gitignore。
- **禁止**在仓库根目录放 `.vsix`。
- 死代码 / 一次性工具 → `99-backup-归档旧文件/`（gitignore，不进 git）。
